...
import static org.example.auth.ListenerAccount.Persistence.*;
...
public class ListenerAccount extends Account {
    ...
    public void buySong(int id) throws SQLException {
        if(hasSong(this.id,id)){
            //nic
        }else{
            if(getCredits(this.id) > 0){
                //sql na dodanie songa
                addCredits(this.id,-1);
            }else{
                throw new NotEnoughCreditsException("za malo kredytow");
            }
        }
    }
